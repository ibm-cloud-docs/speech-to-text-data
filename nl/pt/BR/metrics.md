...
O {{site.data.keyword.speechtotextdatafull}} for {{site.data.keyword.icp4dfull}} pode retornar dois tipos de métricas opcionais para uma solicitação de reconhecimento de voz:
...
As métricas de áudio fornecem informações detalhadas sobre as características de sinal do áudio de entrada. Os resultados fornecem métricas agregadas para o áudio de entrada inteiro na conclusão do processamento da fala. Para um usuário tecnicamente sofisticado, as métricas podem fornecer insights significativos sobre as características detalhadas do áudio.
